...
import { deleteOrder, getDistributorOrders } from "@/features/orders/services/orderService";
...
const DistributorOrderList: React.FC = () => {
  ...
  const fetchOrders = async (): Promise<void> => {
    try { setLoading(true); setError(null); const data = await getDistributorOrders(); setOrders(data as OrderDetailResponse[]); }
    catch (err) { console.error("Error fetching distributor orders:", err); setError("Failed to load distributor orders."); }
  ...
  if (showForm) {
    return <OrderFormView orderId={editingOrderId || viewingOrderId || undefined} orderType="distributor_order" mode={formMode} onSave={handleFormSave} onBack={handleBackFromForm} />;
  }

  return (
    <div className="w-full">
      <div className="flex items-center justify-between mb-5">
        <h2 className="text-[17px] font-semibold text-foreground">Distributor Orders</h2>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 text-center">
          <div className="w-12 h-12 rounded-2xl bg-secondary/60 flex items-center justify-center mb-4"><Truck className="w-6 h-6 text-muted-foreground/50" /></div>
          <h3 className="text-[17px] font-semibold text-foreground mb-1">No Orders Yet</h3>
          <p className="text-[13px] text-muted-foreground mb-5 max-w-[240px]">Create your first distributor order.</p>
