...
import { deletePerson, getDistributorPersons } from "@/features/persons/services/personService";
...
const DistributorList: React.FC = () => {
  ...
  const fetchPersons = async (): Promise<void> => {
    try { setLoading(true); const data = await getDistributorPersons(); setPersons(data as PersonDetailResponse[]); setError(null); }
    catch (err) { setError("Failed to fetch distributors"); console.error("Error fetching distributors:", err); }
  ...
  if (showForm) {
    return <PersonFormView personId={editingPersonId || viewingPersonId || undefined} personType="distributor" mode={formMode} onSave={handleFormSave} onBack={handleBackFromForm} />;
  }

  return (
    <div className="w-full">
      <div className="flex items-center justify-between mb-5">
        <h2 className="text-[17px] font-semibold text-foreground">Distributors</h2>
        <div className="flex gap-2">
          ...
          <Button size="sm" onClick={handleAddPerson} className="h-9 text-[13px]"><Plus className="h-3.5 w-3.5 mr-1.5" />Add Distributor</Button>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 text-center">
          <div className="w-12 h-12 rounded-2xl bg-secondary/60 flex items-center justify-center mb-4"><MapPin className="w-6 h-6 text-muted-foreground/50" /></div>
          <h3 className="text-[17px] font-semibold text-foreground mb-1">No Distributors Yet</h3>
          <p className="text-[13px] text-muted-foreground mb-5 max-w-[240px]">Add your first distributor to get started.</p>
          <Button onClick={handleAddPerson}><Plus className="h-4 w-4 mr-2" />Add Distributor</Button>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
