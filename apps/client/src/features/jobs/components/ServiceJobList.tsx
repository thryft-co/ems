...
import { deleteJob, getServiceJobs } from "@/features/jobs/services/jobService";
import { Job } from "@/features/jobs/types/job";
import DeleteConfirmationDialog from "@/shared/components/DeleteConfirmationDialog";
import { Button } from "@/shared/ui/button";
import JobCard from "./JobCard.tsx";
import JobFormView from "./JobFormView";
import { PlusIcon, RefreshCw, Wrench } from "lucide-react";
...
const ServiceJobList: React.FC = () => {
  ...
  const fetchJobs = async (): Promise<void> => {
    try { setLoading(true); setError(null); const data = await getServiceJobs(); setJobs(data); }
    catch (err) { console.error("Failed to fetch service jobs:", err); setError("Failed to load service jobs."); }
    finally { setLoading(false); }
  };

  useEffect(() => { fetchJobs(); }, []);

  const handleViewJob = (jobId: string): void => { if (!jobId) return; setSelectedJobId(jobId); setFormMode("view"); setShowForm(true); };
  const handleEditJob = (jobId: string): void => { if (!jobId) return; setEditJobId(jobId); setFormMode("edit"); setShowForm(true); };
  const handleDeleteJob = (jobId: string, jobNumber: string): void => { if (!jobId) return; setDeleteJobId(jobId); setDeleteJobNumber(jobNumber); setShowDeleteDialog(true); };
  ...
  if (showForm) {
    ...
    return <JobFormView jobId={jobId || undefined} jobType="service" mode={formMode} onSave={handleFormSave} onBack={handleBackFromForm} />;
  }

  return (
    <div className="w-full">
      <div className="flex items-center justify-between mb-5">
        <h2 className="text-[17px] font-semibold text-foreground">Service Jobs</h2>
        <div className="flex gap-2">
          ...
          <Button size="sm" onClick={handleAddJob} className="h-9 text-[13px]"><PlusIcon className="h-3.5 w-3.5 mr-1.5" />New Job</Button>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 text-center">
          <div className="w-12 h-12 rounded-2xl bg-secondary/60 flex items-center justify-center mb-4"><Wrench className="w-6 h-6 text-muted-foreground/50" /></div>
          <h3 className="text-[17px] font-semibold text-foreground mb-1">No Service Jobs Yet</h3>
          <p className="text-[13px] text-muted-foreground mb-5 max-w-[240px]">Create your first service job to get started.</p>
